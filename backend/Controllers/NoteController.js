    ...
    static async editNote(req,res){
        ...
        try {
            if(note){
                ...
                note.descrition - description || note.description
                note.date = Date.now()
                await note.save();
                res.status(201).json({message: "anotação editada com sucesso", note: note});
            }else {
                res.status(404).json({error: "Erro ao tentar achar uma anotação"})
            }
        } catch (error) {
            console.log (error);
            res.status(404).json({error: "Erro ao tentar editar uma anotção"})
        }
    }
}